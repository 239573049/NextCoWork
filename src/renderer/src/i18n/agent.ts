...
export const agentZh = {
  ...
  'chat.cost.input': '输入 Token 费用',
  'chat.cost.output': '输出 Token 费用',
  'chat.cost.cacheRead': '缓存读取费用',
  'chat.cost.cacheWrite': '缓存写入费用（常规）',
  'chat.cost.cacheWrite1h': '缓存写入费用（1 小时）',
  'chat.cost.other': '其他费用',
  'chat.cost.unpriced': '部分请求未计价，费用不完整',
  'chat.cost.mixedCurrency': '该模型涉及多种币种，无法合计',
  'chat.cost.estimateNote': '分类费用按当前定价估算分摊；总额以请求时冻结的账目为准。不同币种不换算。',
  'chat.cost.loadFailed': '无法读取会话账目',
  ...
  // 收起后长时间无操作缩成的小球:无障碍标签 + title,点击恢复成标题行
  'chat.taskChecklist.restore': '展开任务清单（{done}/{total} 已完成）',
  /*
...
export const agentEn: Record<keyof typeof agentZh, string> = {
  ...
  'chat.cost.input': 'Input token cost',
  'chat.cost.output': 'Output token cost',
  'chat.cost.cacheRead': 'Cache read cost',
  'chat.cost.cacheWrite': 'Cache write cost (standard)',
  'chat.cost.cacheWrite1h': 'Cache write cost (1 hour)',
  'chat.cost.other': 'Other charges',
  'chat.cost.unpriced': 'Some requests are unpriced; cost is incomplete',
  'chat.cost.mixedCurrency': 'This model uses multiple currencies; no combined total',
  'chat.cost.estimateNote': 'Category costs are allocated using current pricing estimates; totals use frozen request charges. Currencies are not converted.',
  'chat.cost.loadFailed': 'Could not load session charges',
  ...
  'chat.taskChecklist.restore': 'Show task checklist ({done}/{total} completed)',
