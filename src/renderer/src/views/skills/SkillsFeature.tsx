...
export function SkillsFeature({
  ...
  const [marketDetail, setMarketDetail] = useState<SkillMarketItem | null>(
    null,
  );
  ...
  const categories = useMemo(
    () => [
      ...new Set(
        (view === "market"
          ? marketCategories.map((category) => ({ category }))
          : items
        )
  ...
  const selectedInstalled =
    selected !== null && items.some((item) => item.name === selected.name);
  ...
  const mineItems = useMemo(
    () => items.filter((item) => item.sourceKind !== "builtin"),
    [items],
  );
  ...
  const recentMineCount = mineItems.filter(
    (item) => item.lastUsedAt !== undefined && item.lastUsedAt >= Date.now() - 30 * 24 * 60 * 60 * 1000,
  ).length;
  const untriggeredMineCount = mineItems.filter((item) => (item.usageCount ?? 0) === 0).length;
  const activeMineCount = mineItems.filter((item) => item.activeInWorkspace && item.globalEnabled).length;
  const issueMineCount = mineItems.filter(
    (item) => (item.diagnostics?.length ?? 0) > 0,
    ...
    <div className="flex min-h-0 flex-1 flex-col bg-canvas">
      ...
      <div className="flex min-h-0 flex-1 flex-col overflow-y-auto px-6 py-5">
        <div className="mx-auto w-full max-w-[1080px]">
          ...
          {view === "market" && <div className="mt-4 flex flex-wrap gap-2">
            <button
              type="button"
              onClick={() => setCategory("")}
              className={cn(
                ...
                <SkillCard
                  ...
                  item={
                    view === "market"
                      ? {
                          ...item,
                          globalEnabled: items.some(
                            (local) => local.name === item.name,
                          ),
                        }
                      : item
                  }
                  ...
                  onInstall={
                    view === "market" &&
                    !items.some((local) => local.name === item.name)
                      ? () => {
          ...
          <div className="space-y-3 text-[12px] text-fg-muted">
            <div className="rounded-[10px] bg-tint p-3">
              ...
              <div className="mt-2 flex items-center justify-between">
                ...
                <span className="text-fg">
                  {marketDetail?.author ?? selected.sourceKind}
                </span>
              </div>
              <div className="mt-2 flex items-center justify-between">
                <span>{t("skills.scopeLabel")}</span>
                <span className="text-fg">
                  {selected.scope ?? t("skills.global")}
                </span>
              </div>
  ...
  return (
    <section className="space-y-4">
      <div className="grid grid-cols-2 gap-2 xl:grid-cols-4">
        <MineStat value={items.length} label={t("skills.totalSkills")} hint={t("skills.totalSkillsHint")} />
        <MineStat value={recentCount} label={t("skills.recentActive")} hint={t("skills.recentActiveHint")} />
        <MineStat value={untriggeredCount} label={t("skills.activeSkills")} hint={t("skills.activeSkillsHint")} />
        <MineStat value={issueCount} label={t("skills.issueSkills")} hint={t("skills.issueSkillsHint")} tone={issueCount > 0 ? "danger" : "default"} />
      </div>
      <div className="max-w-[330px] rounded-[14px] border border-hairline bg-surface p-3.5">
        <div className="flex items-center justify-between text-[12px] font-medium text-fg">
          <span>{t("skills.contextUsage")}</span><span>{contextPercent}%</span>
        </div>
        <div className="mt-2 h-1.5 overflow-hidden rounded-pill bg-tint">
          <div className="h-full rounded-pill bg-accent transition-[width] duration-500" style={{ width: `${contextPercent}%` }} />
        </div>
        <p className="mt-2 text-[11px] text-fg-faint">{t("skills.contextUsageHint", { active: activeCount, total: items.length })}</p>
      </div>
      <div className="flex flex-wrap items-center gap-2">
        <Segmented
          value={scopeFilter}
        ...
        />
        <Select value={statusFilter} onValueChange={(value) => onStatusChange(value as StatusFilter)} ariaLabel={t("skills.statusFilter")} options={statusOptions} className="w-[132px]" />
        <Select value={category} onValueChange={onCategoryChange} ariaLabel={t("skills.categoryFilter")} options={[{ value: "", label: t("skills.allCategories") }, ...categories.map((value) => ({ value, label: value }))]} className="w-[150px]" />
        {diagnostics.length > 0 && <span className="inline-flex items-center gap-1 rounded-pill bg-danger/10 px-2.5 py-1.5 text-[11px] text-danger"><CircleAlert size={12} />{t("skills.diagnosticsCount", { count: diagnostics.length })}</span>}
      </div>
      <div className="overflow-hidden rounded-[14px] border border-hairline bg-surface">
        <div className="hidden grid-cols-[minmax(250px,1.8fr)_130px_130px_90px_110px_82px] gap-3 border-b border-hairline px-3 py-2 text-[11px] text-fg-faint xl:grid">
          <span>{t("skills.tableSkill")}</span><span>{t("skills.scopeLabel")}</span><span>{t("skills.workspaceStatus")}</span><span>{t("skills.triggerCount")}</span><span>{t("skills.lastUsed")}</span><span className="text-right">{t("skills.actions")}</span>
        </div>
        {filtered.length === 0 ? <div className="px-4 py-14 text-center text-[13px] text-fg-faint">{t("skills.empty")}</div> : filtered.map((item) => (
          <div key={item.id} className="grid gap-3 border-b border-hairline px-3 py-3 last:border-b-0 xl:grid-cols-[minmax(250px,1.8fr)_130px_130px_90px_110px_82px] xl:items-center">
            <button type="button" className="min-w-0 text-left" onClick={() => onDetails(item)}>
              <div className="flex min-w-0 items-center gap-2"><span className="truncate text-[13px] font-medium text-fg">{item.displayName ?? item.name}</span>{(item.diagnostics?.length ?? 0) > 0 && <span className="rounded-pill bg-danger/10 px-1.5 py-0.5 text-[10px] text-danger">{t("skills.needsAttention")}</span>}</div>
...
function MineStat({ value, label, hint, tone = "default" }: { value: number; label: string; hint: string; tone?: "default" | "danger" }): ReactNode {
  return <div className="rounded-[14px] border border-hairline bg-surface p-3.5"><div className={cn("text-[24px] font-semibold leading-none tabular-nums", tone === "danger" ? "text-danger" : "text-fg")}>{value}</div><div className="mt-1 text-[12px] font-medium text-fg">{label}</div><div className="mt-1 text-[11px] text-fg-faint">{hint}</div></div>;
}
