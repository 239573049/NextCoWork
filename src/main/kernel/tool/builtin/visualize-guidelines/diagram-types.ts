/**
 * visualize read_me 的 `diagram` 模块规范 —— 逐字取自 Anthropic 的 `visualize:read_me` 响应。
 *
 * 需求:流程图 / 结构图 / 示意图的分类规范（本套规范里最大的一段）。模型在写 widget 之前先读它,而不是每次从零试错 ——
...
export const DIAGRAM_TYPES = `## Diagram types
