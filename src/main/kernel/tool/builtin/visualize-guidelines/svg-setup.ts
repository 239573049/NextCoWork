/**
 * visualize read_me 的 `SVG setup` 模块规范 —— 逐字取自 Anthropic 的 `visualize:read_me` 响应。
 *
 * 需求:SVG 图的工程规范：viewBox 自检、箭头 marker、标定过的字宽表、预置 class。模型在写 widget 之前先读它,而不是每次从零试错 ——
...
export const SVG_SETUP = `## SVG setup
