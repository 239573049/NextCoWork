/**
 * widget 外壳的**基础样式表** —— 逐字取自 Anthropic 的宿主侧样式(经 pi 抠出)。
 *
 * 需求:规范正文让模型写 `class="box"` / `class="t"` / `class="c-blue"` / 裸
 * `<button>` / `<input type="range">`,这些样式**必须由宿主提供** ——
 * 正文里那句"already loaded in SVG widget"指的就是这份表。少了它,一张
 * 完全按规范写的流程图会渲染成**没有描边、没有字号的黑色方块**,而模型
 * 按规范检查过一遍,不会觉得自己写错了。
 *
 * ★ **正文一个字都不许改**(同 `visualize-guidelines/`):里面每个色值都是量出来的。
 *
 * ★ `:root` 里那套变量值是**深色**的兜底值,而宿主每帧还会把当前主题的
 * 具体色值用**内联样式**写到 `documentElement` 上 —— 内联样式优先级高于这里的
 * 选择器,所以实际生效的是应用当前的主题,这一份只是"宿主还没推过来"那几帧
 * 的兜底。**别把这里的色值改成我们的 token 名**,它同时还是 `--p/--s/--t/--b/--bg2`
 * 这五个短名字的定义处,而那五个名字由 `renderer/theme/widget-tokens.ts` 按外观重算。
 *
 * 来源:Michaelliv/pi-generative-ui(MIT)的 `svg-styles.ts`,原作者标注为
 * "reverse-engineered from the claude.ai artifact rendering system"。
 * 正文版权归 Anthropic。
 */
