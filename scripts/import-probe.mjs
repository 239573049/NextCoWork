 ...
 * 2. `--user-data-dir` 必须传,它一个人管着两件事:
 *    - **数据根**。`main/index.ts` 的 `resolveDataRoot()` 认这个开关 —— 传了就以它
 *      为数据根,不传就是用户真实的 `~/.next-cowork`,探针会直接读写开发者自己的库。
 *      (历史注记:早先数据根从 cwd 派生,隔离边界是**测试工作目录**;现在不是了,
 *      cwd 只剩下「工作区在哪」这一层含义。)
 *    - **单实例锁**。锁在 `app.setPath` 之前就取,用的正是命令行这个值。不传的话
 *      已装版本会顶掉这次启动,症状是「DevTools 起来了但没有 page target」。
 *
