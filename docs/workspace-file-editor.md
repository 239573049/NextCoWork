...
- 远端（SSH）工作区没有系统回收站：payload 移入被删项**原父目录**下的 `.next-cowork-trash/`（避开跨设备 rename），索引条目写在工作区根的 `.next-cowork/trash-index/<token>.json`，内容是 `{ originalPath, recoveryPath, deletedAt }`，两者都是工作区相对路径。
- 可恢复项经 `workspace:listRecovery` 从**服务器索引**派生，不存在客户端状态里，因此刷新、切换子树根、断线重连和重启应用后都还在。列举时顺带回收孤儿条目（payload 已不存在，即恢复成功后的自清理），并对原路径已被重新占用的条目标记 `occupied`，UI 据此禁用恢复按钮。恢复复用 `workspace:mutateFile` 的 `move`，并带上列举时的 `environmentKey`：重连换了环境后旧记录会被拒绝。
